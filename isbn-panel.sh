...
export default function ScanPage() {
  ...
  const [startWithKbd,   setStartWithKbd]   = useState(false);
    ...
    <>
      <div className="flex flex-col min-h-screen pb-24" style={{ background: "var(--bg)" }}>
        ...
        <div className="flex flex-col gap-3 px-4">
          {/* Main scan button */}
          <button onClick={() => { setStartWithKbd(false); ready && setScanning(true); }} disabled={!ready}
      ...
      {scanning && library_id && email && (
        <Scanner
          ...
          userEmail={email}
          collections={collections}
          startWithKeyboard={startWithKbd}
          onSuccess={handleSuccess}
