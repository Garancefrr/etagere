...
async function fromGoogleBooks(isbn: string): Promise<LookupResult | null> {
  try {
    ...
    if (!res.ok) {
      console.error("Google Books error:", res.status, await res.text().catch(() => ""));
      return null;
    }
    ...
    return {
      isbn, title: vol.title, authors: vol.authors ?? [],
      cover_url: coverUrl,
...
// ── Main lookup ───────────────────────────────────────────────────────────────
// Priority:
// 1. Google Books (best coverage + images, now with API key = 1000 req/day free)
// 2. BnF (excellent for French books with UNIMARC series data)
// 3. Open Library (fallback, sometimes wrong metadata)

export async function lookupISBN(code: string): Promise<LookupResult | null> {
  const clean = code.replace(/[-\s]/g, "");

  // Try Google Books first — best coverage with API key
  const gb = await fromGoogleBooks(clean);
  if (gb?.title) return gb;

  // Try BnF for French/European EAN-13 codes
  if (/^\d{13}$/.test(clean)) {
    const bnf = await fromBnF(clean);
    if (bnf?.title) return bnf;
  }

  // Open Library as last resort
