...
export default function StatsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/library?user_id=${session.user.id}`)
      .then(r => r.json())
      .then(({ id }) => fetch(`/api/books?library_id=${id}`))
      .then(r => r.json())
      .then(setBooks)
      .catch(() => {})
  ...
  if (stats.total === 0) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 pb-24" style={{ background: "var(--bg)" }}>
      ...
      <p className="text-center" style={{ fontSize: 14, color: "var(--txt3)" }}>Scannez vos premiers livres pour voir vos statistiques</p>
      <BottomNav />
    </div>
  );
