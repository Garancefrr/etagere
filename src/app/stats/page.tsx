...
export default function StatsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const enCours = books.filter(b => b.status === "en_cours");
    const aLire   = books.filter(b => b.status === "a_lire");

    // Pages: ALL read books (total lifetime)
    ...
    // Date helper: use finished_at if available, otherwise added_at for "lu" books
    const readDate = (b: typeof books[0]) => b.finished_at ?? b.added_at;
    ...
    // Monthly reading (books finished per month this year)
    ...
    const favType = typeMax === 0 ? null : livres === typeMax ? "Livres" : bds === typeMax ? "BD" : "Manga";
    ...
    return {
      ...
      en_cours: enCours.length, a_lire: aLire.length,
      pages, pagesThisYear, avg,
      livres, bds, mangas, favType,
    ...
    <div className="min-h-screen pb-24" style={{ background: "var(--bg)" }}>
      ...
      <div className="grid grid-cols-2 gap-3 px-3 mb-4">
        {[
          ...
          { icon: FileText,   label: "Pages lues",          value: stats.pages > 0 ? stats.pages.toLocaleString("fr") : "—",   sub: `${stats.pagesThisYear > 0 ? stats.pagesThisYear.toLocaleString("fr") + " cette année" : "au total"}`, color: "#FB923C" },
          { icon: Star,       label: "Note moyenne",        value: stats.avg ?? "—",                                           sub: ratings_sub(stats),                              color: "#FBBF24" },
...
function ratings_sub(stats: { lu: number; avg: string | null }) {
  if (!stats.avg) return "pas encore de note";
  return `sur ${stats.lu} ${stats.lu > 1 ? "livres notés" : "livre noté"}`;
}
