...
export async function GET(req: NextRequest) {
  ...
  // If job is running and has rows to process, do a batch
  if (job.status === "running" && job.rows_data) {
    try {
      ...
      for (const row of batch) {
        try {
          // Quick cover search (short timeout)
          ...
          await insertBook({
            ...
            rating: row.rating, added_by: profileId ?? undefined,
          } as any);
  ...
  // Return current status (no processing needed)
  return NextResponse.json({
