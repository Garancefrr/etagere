#!/bin/bash
set -e
echo "🖼️ Récupération images manquantes depuis la biblio..."
cd "$(git rev-parse --show-toplevel)"
mkdir -p src/app/api/books/refresh-covers
cat > "src/app/api/books/refresh-covers/route.ts" << 'FILEOF'
...
FILEOF
git add -A
git commit -m "feat: refresh missing covers for existing library books"
git push
echo "🎉 Déployé !"
