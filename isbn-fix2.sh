#!/bin/bash
set -e
echo "🔍 Fix ISBN lookup + clavier auto + ISBN-10 support..."
...
export async function lookupISBN(code: string): Promise<LookupResult | null> {
  ...
  // Try Google Books for any ISBN (10 or 13 digits)
  if (isISBN(clean)) {
  ...
  if (ol?.title) {
    if (!isISBN(clean)) ol._unreliable = true;
    return ol;
  }
...
FILEOF
git add -A
git commit -m "fix: ISBN-10 support, always show keyboard on not_found, fix doubt detection"
git push
echo "🎉 Déployé !"
