#!/bin/bash
set -e
echo "🔧 Fix création collection — règle auteur < 2 livres respectée..."
...
export default function Scanner({ rapidMode, libraryId, userEmail, collections, startWithKeyboard, onSuccess, onClose }: Props) {
  ...
  const doSave = useCallback(async (
    ...
    try {
      ...
      // Only create collection if the lookup explicitly flagged it
      const shouldCreateCollection =
        r.book._createCollection === true ||
        (seriesName && seriesIndex && (bt === "bd" || bt === "manga")); // manual series+tome entry
...
FILEOF
git add -A
git commit -m "fix: no collection for books unless saga or prolific author (>2 books)"
git push
echo "🎉 Déployé !"
